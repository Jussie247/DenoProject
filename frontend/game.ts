document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchLeaderboard() {
        try {
            ...
            if (!res.ok) return;
            const rows: Array<{ user: string; score: number }> = await res.json();
            lbList.innerHTML = "";
            rows.forEach((r) => {
    ...
    async function submitHighScore(newHigh: number) {
        try {
            await fetch("/score", {
                method: "POST",
    ...
    backpack.addEventListener("click", async () => {
        if (inventoryDiv.classList.contains("hidden")) {
            ...
            const items: Array<{ emoji: string; count: number }> = await res.json();
